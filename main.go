...
func main() {
	...
	receiver := func(done <-chan int, instream <-chan int) {
		go func() {
			for {
				...
				case <-done:
					{
						fmt.Println("[log] Завершаем горутину получения данных из пайплайна...")
						return
					}
				}
			}
	...
	filter1 := func(done <-chan int, input <-chan int) <-chan int {
		...
		go func() {
			for {
				...
				case <-done:
					{
						fmt.Println("[log] Завершаем горутину фильтрации отрицательных значений...")
						return
					}
				}
			}
	...
	filter2 := func(done <-chan int, input <-chan int) <-chan int {
		...
		go func() {
			for {
				...
				case <-done:
					{
						fmt.Println("[log] Завершаем горутину проверки на кратность трем...")
						return
					}
				}
			}
		...
		go func() {
			for {
				...
				case n := <-input:
					buff.Value = n
					buff = buff.Next()
					fmt.Printf("[log] Добавили %v в буфер\n", n)
				case <-done:
					return
				}
			}
	...
	case <-complete:
		{
			fmt.Println("[log] Разблокируется основная горутина (выход из программы)...")
			// Небольшой тайм-аут для отработки выходов горутин пайплайна
